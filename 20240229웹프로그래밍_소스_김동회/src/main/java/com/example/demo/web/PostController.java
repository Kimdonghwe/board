...
import com.example.demo.web.form.res.ApiProductResponse;
...
public class PostController {
  ...
  public ApiProductResponse<?> getComment(@PathVariable("pid") Long postId){

    Optional<Post> findedpost = postSVC.findById(postId);
    Post post = findedpost.orElseThrow();

    // 선택한 상품아이디를 받아와서 상품에대한 모든 댓글 호출
    List<Comment> comments = commentSVC.findbyIdAll(postId);

    log.info("comments = {}", comments);

    // 선택한 상품과 해당상품에대한 댓글을 객체로 감싸서 body로 만든다.
    AllData allData = new AllData(post,comments);
